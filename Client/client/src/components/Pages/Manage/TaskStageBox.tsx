...
const TaskStageBox: React.FC<TaskStageBoxProps> = ({
  ...
  return (
    <div className="flex flex-col bg-gray-300 p-4 rounded-lg shadow-lg w-full transition-transform transform hover:scale-105">
    ...
    <div className="flex flex-col space-y-3 w-full h-64 overflow-y-auto">
      {sortedTasks.length ? (
        sortedTasks.map((task) => (
          <div
            key={task._id}
            className="flex justify-between items-center p-4 bg-white w-[600px] sm:w-full rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300"
          >
