...
const TaskStageBox: React.FC<TaskStageBoxProps> = ({ title, tasks, onEditTask, onDeleteTask, onMoveTask }) => {
  ...
  return (
    <div className="flex flex-col bg-gray-300 p-4 rounded-lg shadow-lg transition-transform w-full transform hover:scale-105">
      <h2 className="text-xl font-semibold text-gray-800 mb-4 border-b-2 w-full border-blue-600 pb-2">{title}</h2>
      <div className="flex flex-col space-y-3 w-full overflow-x-auto">
        {sortedTasks.length ? (
          sortedTasks.map((task) => (
            <div key={task._id} className="flex justify-between items-center p-4 bg-white w-[160%] sm:w-full rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
